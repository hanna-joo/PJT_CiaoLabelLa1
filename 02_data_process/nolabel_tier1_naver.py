...
naver = spark.read.option("multiline", "true").option("header", "true").csv("pjt3/raws/nolabel_naver.csv") \
    ...
    .drop(col('del_labels')) \
    .withColumn('id2', f.concat(col('id'), f.lit('01'))) \
    ...
    .withColumn('proc1',
                f.regexp_replace('title', r'((\d\.\d{1,3}|\d)+(ML|L|리터))*((\d)+(개입|개|입|병|펫|캔|본|EA|PET|CAN|박스|페트))*',
                                 '')) \
    .withColumn('proc2', f.regexp_replace('proc1', r'( x | X )', ' ')) \
    .withColumn('proc3',
                f.regexp_replace('proc2', r'([(\[★][a-zA-Z0-9가-힣 ]*[\])★])*([-=+,#/?:^$@*"※~&%ㆍ!|()\[\]<>`…_★]+)*', '')) \
    .withColumn('proc3', f.regexp_replace('proc3',
                                          r'(무라벨)*(라벨프리)*(본사직영)*(노라벨)*(생수배달)*(정기배달)*(배달전문)*(다운로드쿠폰증정)*(라벨없는)*(생수배송전문)*(생수전문배송)*(상세설명참조)*',
                                          '')) \
...
naver.write.format('parquet').mode('overwrite').save('pjt3/first/nolabel_naver1.parquet')
