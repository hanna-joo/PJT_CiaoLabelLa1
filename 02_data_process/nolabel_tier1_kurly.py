...
kurly = spark.read.option("multiline", "true").option("header", "true").csv("pjt3/raws/nolabel_kurly.csv") \
    ...
    .drop(col('del_labels')) \
    .withColumn('id2', f.concat(col('id'), f.lit('03'))) \
    ...
    .withColumn('proc3', f.regexp_replace('proc2', r'([\[\]()])*', '')) \
    .withColumn('proc3', f.regexp_replace('proc3', r'(무라벨)*(라벨프리)*(본사직영)*(노라벨)*(라벨없는)*(생수배송전문)*(생수전문배송)*(상세설명참조)*', '')) \
...
kurly.write.format('parquet').mode('overwrite').save('pjt3/first/nolabel_kurly1.parquet')
